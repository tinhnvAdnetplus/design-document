...
_TRANSITIONS: dict[SessionState, frozenset[SessionState]] = {
    ...
    SessionState.READY: frozenset({SessionState.BUSY, SessionState.UNAVAILABLE, SessionState.RECOVERY_REQUIRED, SessionState.TERMINATING}),
    SessionState.BUSY: frozenset({SessionState.READY, SessionState.UNAVAILABLE, SessionState.RECOVERY_REQUIRED, SessionState.TERMINATING}),
...
class SessionSupervisor:
    ...
    def start(self, spec: SessionSpec, *, readiness_timeout: float = 30) -> SessionObservation:
        ...
        if existing is not None:
            if existing.adapter_version != spec.adapter_version or existing.launch_sha256 != launch_sha:
                self._transition(existing, SessionState.RECOVERY_REQUIRED, diagnostic={"reason": "adapter_or_launch_drift"})
                raise SessionRecoveryRequiredError("stale adapter version or launch contract")
    ...
    def observe(self, spec: SessionSpec) -> SessionObservation:
        ...
        if live and not identity_matches and record.state not in {
        ...
        ready = identity_matches and record.state in {
            SessionState.READY,
            SessionState.BUSY,
        }
        diagnostic = None
    ...
    def _read_turn(self, record: SessionRecord, request: TurnRequest, request_path: Path, response_path: Path, *, reconciled: bool) -> TurnObservation:
        ...
        evidence = {
            ...
            "diagnostic_redacted": (
                f"stderr_present bytes={len(stderr.encode('utf-8'))}"
                if value.get("exit_code") and stderr
                else ""
            ),
        }
    ...
    def terminate(self, session_id: str, *, grace_seconds: float = 2) -> SessionRecord:
        ...
        return self._transition(record, SessionState.TERMINATED, diagnostic={"terminal_absent": not self._live(record)})
